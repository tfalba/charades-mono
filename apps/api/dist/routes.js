...
exports.router.post("/challenge", async (req, res) => {
    try {
        const { theme = "movies", difficulty = "Medium" } = (req.body ??
            {});
        const existingPrompts = (0, storage_1.getStoredPrompts)(theme);
        const seenNormalized = new Set(existingPrompts.map(storage_1.normalizePrompt));
        ...
        const messages = [
            ...
            {
                role: "user",
                content: existingPrompts.length === 0
        ...
        for (let attempt = 0; attempt < maxAttempts && collected.length < targetCount; attempt++) {
            ...
            for (const raw of items) {
                ...
                if (!item)
                    continue;
                const normalized = (0, storage_1.normalizePrompt)(item);
        ...
        (0, storage_1.rememberPrompts)(theme, collected);
        const payload = { ok: true, items: collected };
        res.json(payload);
    }
    catch (e) {
        console.error(e);
